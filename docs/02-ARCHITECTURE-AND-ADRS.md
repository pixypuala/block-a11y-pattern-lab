...
- Pattern contract schema describing required states, events, focus transitions and accessible naming.
- Adapter interface that maps contract states to implementation events.
- Test-result schema recording browser, OS, assistive technology, version, steps, observations and limitations.
- Stable CSS hooks only where necessary; behavior must not depend on class-name accidents.
